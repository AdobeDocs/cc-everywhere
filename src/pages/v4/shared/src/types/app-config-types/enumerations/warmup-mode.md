...
| `PRELOAD` | `"preload"` | Preload mode for intents that support it (EDIT_IMAGE_V2, ANNOTATE_IMAGE, EDIT_DESIGN). This mode preloads the entire workflow in a hidden iframe, allowing for instant transitions when the actual action is launched. The preloaded state is tracked and reused on launch. |
