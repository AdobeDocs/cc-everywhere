...
| `isPromptSafe?` | (`prompt`: `string`) => `Promise` [`PromptSafetyResult`](prompt-safety-result.md) | Callback to check if a prompt is safe for image generation. This allows partners to implement their own prompt moderation logic. | - |
