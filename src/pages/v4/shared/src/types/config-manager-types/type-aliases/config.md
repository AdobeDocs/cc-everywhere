...
Config type for the SDK. It contains the environment, enable debug analytics, and the clients.

## See

 - [Environment](../../host-info-types/enumerations/environment.md)
...
| `CCX.EDITOR.QUERY_PARAMS.KEYS.MODE` | `string` | - |
...
| `IMS.BODY_PARAMS.VALUES.TARGET_RESPONSE_TYPE_CODE` | `string` | - |
| `IMS.BODY_PARAMS.VALUES.TARGET_RESPONSE_TYPE_TOKEN` | `string` | - |
...
| `GENSTUDIO_CMN` | `object` | - |
| `GENSTUDIO_CMN.CLIENT_ID` | `string` | - |
| `HALO` | `object` | - |
...
| `AEP` | `object` | Adobe Experience Platform config for pages that do not already load alloy via Adobe Launch (e.g. 3P partner pages). On 1P Adobe pages the existing `window.alloy` global is used instead; these values are ignored there. Set `DATASTREAM_ID` and `ORG_ID` per environment; leave both empty to disable own-instance init while still falling back to `window.alloy`. |
| `AEP.DATASTREAM_ID` | `string` | - |
| `AEP.ORG_ID` | `string` | - |
