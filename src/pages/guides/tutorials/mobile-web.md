...
![Mobile Web Hero Image](./images/mobile-web--hero.png)
...
The Web experience is **served by an HTTPS [Express.js](https://expressjs.com/) server running locally**. It belongs to `Mobile-web/server/index.js` and uses the self-signed certificate and key pair generated by `npm run gen:cert` (stored in `Mobile-web/certs`).
...
- `platform=ios` (or any casing, e.g. `iOS`) → `iOS`
- `platform=android` (any casing) → `android`
...
- `window.__PLATFORM__ = "<resolved-platform>"`
...
At this point, you should have tunneled the development server through `ngrok`, as we discussed in [this section](#132-tunnel-the-development-server); this is necessary to allow the mobile applications to properly access the Web experience. Tunneling is only required when developing locally—if you have a remote test or production environment, you should skip this step.

When you access the ngrok URL for the first time, you may see a prompt asking you to confirm that you want to proceed. This is expected behavior—simply click **Visit Site** to continue.

![Accept to visit the ngrok URL](./images/mobile-web--ngrok-warning.png)

## 3. Build the Android application

### 3.1 Overview

The Android application is found in the `Android` folder; it's built using [Kotlin](https://kotlinlang.org/) and uses a [WebView](https://developer.android.com/reference/android/webkit/WebView) to display the Web experience built [in the previous section](#2-build-the-web-experience). The app UI is intentionally simple: it shows one button that loads the [ngrok-tunneled web server URL](#132-tunnel-the-development-server).

### 3.2 Point the app at your tunneled URL

In `MainActivity.kt`, replace the ngrok URL in `START_GAME_URL` with the URL you noted down in [Section 1.3.2](#132-tunnel-the-development-server), and keep the `?platform=android` query parameter.

**That query parameter is important**: as explained in [Section 2.1](#21-the-server), the server uses it to inject `window.__PLATFORM__ = "android"` into the HTML so the client can pass the correct platform metadata to the Embed SDK for analytics purposes.

```kotlin-data-line="4"
// Android/app/src/main/java/com/example/embedsdk/MainActivity.kt
companion object {
    private const val START_GAME_URL =
        "https://your-ngrok-url.ngrok-free.dev?platform=android" // 👈 Replace with your ngrok URL
}
```
...
Build and run the Android project from Android Studio, then tap **Start the Game**. You should see the [same Web experience](#131-install-dependencies-and-run-the-development-server), now running inside a WebView.
...
![Android project running](./images/mobile-web--android-experience.png)

## 4. Build the iOS application

### 4.1 Overview

The iOS application is found in the `iOS` folder; it's built using [Swift](https://developer.apple.com/swift/) and uses a [`WKWebView`](https://developer.apple.com/documentation/webkit/wkwebview) to display the same [Web experience](#2-build-the-web-experience).

Just like the Android sample, the iOS app renders a simple UI with a button that loads the [ngrok-tunneled](#132-tunnel-the-development-server) web server URL into the WebView.

### 4.2 Point the app at your tunneled URL

In `iOS/DevxSampleApp/ViewController.swift`, replace the URL used to load the WebView with the ngrok URL you noted down [here](#132-tunnel-the-development-server), and keep the `?platform=ios` **query parameter**. As explained [earlier](#21-the-server), the server injects `window.__PLATFORM__ = "iOS"` into the HTML so the client can pass the correct platform metadata to the Embed SDK for analytics purposes.
...
Build and run the iOS project from Xcode, then tap **Start the Game**. You should see the same [Web experience](#131-install-dependencies-and-run-the-development-server), now running inside a `WKWebView`.
...
![iOS project running](./images/mobile-web--ios-experience.png)

## Code samples

The code samples for this tutorial (Web Server and Client, Android and iOS applications) are available in the [embed-sdk-mobile-web-tutorial](https://github.com/AdobeDocs/embed-sdk-samples/tree/main/code-samples/tutorials/embed-sdk-mobile-web) repository on GitHub.
...
- [`skipBrowserSupportCheck`](../../v4/shared/src/types/host-info-types/interfaces/config-params-base.md#properties)
- [`appConfig.metaData`](https://cc-embed.adobe.com/docs/v4/release/3p/interfaces/shared_src_types_3p_module_AppConfig_types.TextToImageAppConfig.html#metaData)
