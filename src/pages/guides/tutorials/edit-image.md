...
This is just a workaround to make the local URL work. In production, you'll need to use presigned URLs, or set the headers in your server.
...
// Same Edit Button click handler as before!
...
When the user clicks the **Save button** in the Edit Image experience, the edited image must be displayed in the `<img>` element. First of all, the buttons are defined in the `exportConfig` object, as follows.
