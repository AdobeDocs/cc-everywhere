...
function getFiles(fileExtensions, subDir = 'src/pages') {
    ...
    // Ensure forward slashes for glob
    const pattern = `${subDir}/**/*.{${exts}}`.replaceAll('\\', '/');
    
    // console.log(`[DEBUG] glob pattern: ${pattern}, cwd: ${rootDir}`);
    
    // Use cwd option so glob returns paths relative to root (e.g., src/pages/v4/...)
    // This matches what renameFiles.js expects (paths starting with src/)
