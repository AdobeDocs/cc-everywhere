...
function normalizeLinksInMarkdownFile(file, files) {
  ...
  let data = fs.readFileSync(absFile, "utf8");
  ...
  [...links].forEach((link) => {
    ...
    // path.relative from relativeToDir (which is relative to root)
    // We want result to be relative to relativeToDir
    // path.resolve(relativeToDir, to) assumes CWD if relativeToDir is relative.
    // So we should use absolute paths for calculation.
    
    // relativeToDir is e.g. src/pages/v4
    // absolute is /path/to/repo/src/pages/v4/../foo.md
    
    const relative = path.relative(path.resolve(ROOT_DIR, relativeToDir), absolute);
    to = relative;
    ...
    const potentialFileExtensions = relativeFiles
      .filter((file) => removeFileExtension(file) === to)
